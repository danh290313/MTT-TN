			                            ...
			                            <li data-id="1686182" class="onlineResume">
		                                    ...
		                                    <div class="resumeIntro">
		                                        <h3 class="unread">
			                                        ...
			                                        <span class="fr">Thời gian: ${resume.createTime!""}</span>
			                                        <div> 
		                                    			Tên: ${resume.user.username!""}  / giới tính：${resume.user.sex!""}  / Bằng cấp：${resume.user.degree!""}  / kinh nghiệm：${resume.user.workExperience!""}			                                            			                                        		<br>
			                                        ...
			                                        <div class="jdpublisher">
				                                        <span>
				                                        	xin việc: <a title="${resume.position.name!""}" href="/home/position/detail?id=${resume.position.id!""}">${resume.position.name!""}</a>
				                                        </span>
			                                        </div>
			                                    </div>
			                                    <div class="links">
			                                ...
			                                <div class="contactInfo">
			                                	<span class="c9">Điện thoại:</span>${resume.user.mobile!""}   &nbsp;&nbsp;&nbsp;   
			                                    <span class="c9">Email:</span>${resume.user.email!""}
			                                </div>
			                            </li>
...
<script type="text/javascript">
	...
	function unsuitable(i){
		$.ajax({
			...
			success:function(data){
				if(data.code == 0){
					$("#successMsg").html("Hồ sơ đã được đặt thành không phù hợp!");
					$.colorbox({inline:true, href:$("#successTip"),title:"Thông báo"});
					setTimeout(function(){  
							window.location.reload();//页面刷新
							},3000);
				}else{
						$("#errorMsg").html("设置不合适失败，"+data.msg+"!!!");
						$.colorbox({inline:true, href:$("#errorTip"),title:"Thông báo"});
					}
			}
		});
	...
	function deleteResume(i){
		if(confirm("是否确定要删除改简历？")){
			$.ajax({
				...
				success:function(data){
					if(data.code == 0){
						$("#successMsg").html("Hồ sơ được xóa thành công!");
						$.colorbox({inline:true, href:$("#successTip"),title:"Thông báo"});
						setTimeout(function(){  
								window.location.reload();//页面刷新
								},3000);
					}else{
							$("#errorMsg").html("简历删除失败，"+data.msg+"!!!");
							$.colorbox({inline:true, href:$("#errorTip"),title:"Thông báo"});
						}
				}
			});
